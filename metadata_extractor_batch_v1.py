...
def iter_xml_filelist(filelist):
    """
    Yield XML files from a text file containing one XML path per line.
    This is useful for SLURM array jobs.
    """
...
def write_metadata_batch(xml_paths, out_tsv, append=False, progress_every=10000):
    """
...
def main():
    ...
    input_group.add_argument(
        "--filelist",
        help="Text file containing one XML file path per line"
    )
    ...
    if args.filelist:
        xml_iter = iter_xml_filelist(args.filelist)
    else:
